...
let make_http_call (inputObject : InputData.t) client =
  ("making http call for url: " ^ inputObject.metadataUrl ^ "\n"
  |> Lwt_io.(write stdout))
  >>= fun _ ->
      ...
      if code == 200 then
        Lwt.catch
          (fun () ->
            ...
            |> Lwt.map (fun body -> Some (inputObject, body)))
          (function
            | exn ->
                exn |> Printexc.to_string
                |> Lwt_io.(write stdout)
    ...
    (fun () ->
      Metadata.(
        Yojson.Basic.Util.(
          {
            id =
              json |> member "id" |> to_string_option
            ...
            gridId =
              json |> member "properties" |> member "gridId" |> to_string_option
              |> Option.value ~default:"";
            gridX =
              json |> member "properties" |> member "gridX" |> to_int_option
              |> Option.value ~default:0;
            gridY =
              json |> member "properties" |> member "gridY" |> to_int_option
              |> Option.value ~default:0;
            forecastHourly =
              json |> member "properties" |> member "forecastHourly"
              |> to_string_option |> Option.value ~default:"";
            forecastZone =
              json |> member "properties" |> member "forecastZone"
              |> to_string_option |> Option.value ~default:"";
            county =
              json |> member "properties" |> member "county" |> to_string_option
              |> Option.value ~default:"";
            radarStation =
              json |> member "properties" |> member "radarStation"
              |> to_string_option |> Option.value ~default:"";
            city =
              json |> member "properties" |> member "relativeLocation"
              |> member "properties" |> member "city" |> to_string_option
              |> Option.value ~default:"";
          }
...
let get_metadata (inputObjectList : InputData.t list) =
  let rec throttled_get uri =
    ...
    if allowed then uri |> Client.get
    else
