module Metadata = struct
  ...
  let to_string t =
    "{ " ^ "id='" ^ t.id ^ "'; " ^ "latitude="
    ^ (t.latitude |> Float.to_string)
    ^ ";" ^ "longitude="
    ^ (t.longitude |> Float.to_string)
    ^ ";" ^ "metadataUrl='" ^ t.metadataUrl ^ "'; " ^ "trailhead='"
    ^ t.trailhead ^ "'; "
    ^ (t.trailheadUrl
      |> Option.fold ~none:"" ~some:(fun v -> "trailhead-url='" ^ v ^ "'; "))
    ^ "gridId='" ^ t.gridId ^ "'; " ^ "gridX=" ^ Int.to_string t.gridX ^ "; "
    ^ "gridY=" ^ Int.to_string t.gridY ^ "; " ^ "forecastHourly='"
    ^ t.forecastHourly ^ "'; " ^ "forecastZone='" ^ t.forecastZone ^ "'; "
    ^ "county='" ^ t.county ^ "'; " ^ "radarStation='" ^ t.radarStation ^ "'; "
    ^ "city='" ^ t.city ^ "'; " ^ "}"
...
module InputData = struct
  ...
  let to_json t =
    ...
    let fieldsList =
      [ latitudeTuple; longitudeTuple; metadataUrlTuple; trailheadTuple ]
    in
    `Assoc
      (Option.fold t.trailheadUrl ~none:fieldsList ~some:(fun v ->
           List.concat [ fieldsList; [ ("trailheadUrl", `String v) ] ]))
  ...
  let from_json t =
    ...
    |> List.map (fun x ->
           ...
           let trailheadUrlOpt =
             x |> member "trailead_url" |> to_string_option
           in
           {
             latitude;
             longitude;
